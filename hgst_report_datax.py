...
import sys
print("\n")
print('This is \'Beta\' version 1.1 . This utility collects useful information from the Data60 and Data102 JBODs. This version doesn\'t verify HTTP Certificate. Press Ctrl+C if you do not wish to continue.')
print("\n")
...
print("Host port:")
#print("Status: ",r.status_code)
hostports=len(json_data["Oem"]["WDC"]["SASHostPorts"])
for i in range(0,hostports):
	print("Host Port:"+str(json_data["Oem"]["WDC"]["SASHostPorts"][i]["Id"])+" Connected: "+str(json_data["Oem"]["WDC"]["SASHostPorts"][i]["IsCableConnected"]))
...
sys.stdout = open(os.path.join(os.getcwd(),outfile), "a")
expanders=len(json_data["Oem"]["WDC"]["SASExpanders"])
print()
print("Expanders:")
for i in range(0,expanders):
	print(str(json_data["Oem"]["WDC"]["SASExpanders"][i]["Name"]),str(json_data["Oem"]["WDC"]["SASExpanders"][i]["Id"]), str(json_data["Oem"]["WDC"]["SASExpanders"][i]["FirmwareVersion"]),str(json_data["Oem"]["WDC"]["SASExpanders"][i]["Identifiers"]["DurableName"]),str(json_data["Oem"]["WDC"]["SASExpanders"][i]["Status"]))
